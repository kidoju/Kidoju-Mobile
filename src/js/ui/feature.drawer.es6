...
const feature = {
    ...
    onDrawerListViewClick(e) {
        ...
        switch (command) {
            ...
            case 'scan':
                ...
                break;
            case 'activities':
                controller.application.navigate(`${CONSTANTS.HASH}${VIEW.ACTIVITIES}?language=${encodeURIComponent(language)}&userId=${encodeURIComponent(userId)}`);
                break;
            case 'settings':
                controller.application.navigate(`${CONSTANTS.HASH}${VIEW.SETTINGS}?userId=${encodeURIComponent(userId)}`);
                break;
            case 'help':
                feature.openHelp();
                break;
        ...
        if (barcodeScanner.ready()) {
            barcodeScanner.scan(
                ...
                result => {
                    ...
                    if (!result.cancelled) {
                        assert.type(CONSTANTS.STRING, result.text, assert.format(assert.messages.type.default, 'result.text', CONSTANTS.STRING));
                        ...
                        if (Array.isArray(matches) && matches.length > 2) {
                            ...
                            if (viewModel.get(VIEW_MODEL.LANGUAGE) === language) {
                                ...
                                app.notification.warning(__('notifications.scanLanguageWarning'));
                            }
                        } else {
                            app.notification.warning(__('notifications.scanMatchWarning'));
                        }
                    }
                },
                // Error callback
    ...
    openHelp() {
        ...
        assert.match(CONSTANTS.RX_URL, helpUrl, assert.format(assert.messages.match.default, 'config.constants.helpUrl', CONSTANTS.RX_URL));
        logger.debug({
