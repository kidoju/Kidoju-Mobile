...
;(function (win, $, undefined) {
    ...
    app.controller = {
        ...
        localize: function() {
            ...
            //Main
            $('#main').attr('data-title', app.culture.MAIN_VIEW_TITLE);

            //Search
            $('#search').attr('data-title', '\u00A0'); //Cannot hide title and keep navbar height + &nbsp; is escaped
            $('#search-input').attr('placeholder', app.culture.SEARCH_INPUT_PLACEHOLDER);

            //Categories
            $('#categories').attr('data-title', app.culture.CATEGORIES_VIEW_TITLE);

            //Activities
            $('#activities').attr('data-title', app.culture.ACTIVITIES_VIEW_TITLE);

            //Settings
            $('#settings').attr('data-title', app.culture.SETTINGS_VIEW_TITLE);
            $('label[for="settings-user"]').prepend(app.culture.SETTINGS_USER_LABEL);
            $('label[for="settings-language"]').prepend(app.culture.SETTINGS_LANGUAGE_LABEL);

            //Drawer
            $('#menu-drawer').attr('data-title', app.culture.DRAWER_VIEW_TITLE);
            $('li#menu-drawer-main>a').html(app.culture.MAIN_VIEW_TITLE);
            $('li#menu-drawer-search>a').html(app.culture.SEARCH_VIEW_TITLE);
            $('li#menu-drawer-categories>a').html(app.culture.CATEGORIES_VIEW_TITLE);
            $('li#menu-drawer-activities>a').html(app.culture.ACTIVITIES_VIEW_TITLE);
            $('li#menu-drawer-settings>a').html(app.culture.SETTINGS_VIEW_TITLE);
        },
