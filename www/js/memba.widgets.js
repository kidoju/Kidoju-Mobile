...
(function ($, undefined) {
    ...
    /**
     * Enable binding the readonly attribute of a MultiInput widget
     * @type {*|void}
     */
    kendo.data.binders.widget.readonly = kendo.data.Binder.extend({
        ...
        refresh: function() {
            var that = this,
                ...
                widget = kendo.widgetInstance($(that.element));
            if (widget instanceof ui.MultiInput) {
                widget.readonly(value); //update our widget
            } else if ($.type(widget.readonly) === FUNCTION) { //try not to mess with other widgets
                try {
